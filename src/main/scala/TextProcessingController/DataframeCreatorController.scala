...
class DataframeCreatorController() extends Serializable {
  ...
  def getProductDescriptionDataframe(inputFilePath : String, sparkSession : SparkSession, sc : SparkContext) : DataFrame = {
    ...
    //Step 3: Stemming the DF with Porter's Stemmer Algorithm

//    val stemmed = new Stemmer()
//      .setInputCol("rFilteredProductDescriptionWords")
//      .setOutputCol("stemmed_words")
//      .setLanguage("English")
//
//    val stemmedDescriptionDF = stemmed.transform(filteredWordsDf)
...
//    val stemmedDescriptionDF = stemmed.transform(filteredWordsDf)
  ...
  def getIDF(trainDataframe : DataFrame, sparkSession : SparkSession, sc : SparkContext) : DataFrame = {
    ...
    /** rFilteredProductTitleWords contains only the words from product title, thats why the prediction of NB is 0.
      * in rFilteredProductTitleWords we will add all the generated words from other files, to increase the training
  ...
  def uniteTwoDataframeTexts(trainingDataframe : DataFrame, testDataframe : DataFrame,  sparkSession : SparkSession,sc : SparkContext) : DataFrame = {
    ...
    changedRenamedTrainingRDD.take(2).foreach(println)
    print("changedRenamedTestRDD: ")
    changedRenamedTestRDD.take(2).foreach(println)
    val joinedRDDResult = changedRenamedTrainingRDD.leftOuterJoin(changedRenamedTestRDD)
    print("joinedRDDResult: ")
    joinedRDDResult.take(2).foreach(println)
    ...
    print("joinedRDDResult count : "+joinedRDDResult.count())
    print("changedRenamedTestRDD count : "+changedRenamedTestRDD.count())
    print("changedRenamedTrainingRDD count : "+changedRenamedTrainingRDD.count())
    ...
    print("relevence count : "+relevence.count())
    relevence.take(5).foreach(println)
    relevence.printSchema()

    var relevence2 = relevence.withColumn("rowId1", monotonically_increasing_id())
    //    relevence2.take(5).foreach(println)
    //    relevence2.printSchema()

    var finaResultDF2 = finaResultDF.withColumn("rowId2", monotonically_increasing_id())
    print("finaResultDF2 count : "+finaResultDF2.count())
    finaResultDF2.take(5).foreach(println)
    finaResultDF2.printSchema()
    ...
    print("joinedWithRelevanceFinalDF count : "+joinedWithRelevanceFinalDF.count())
